...
import {
  ...
  useBulkCreatePeriodAttendanceMutation,
  useGetSubjectsQuery,
...
const PeriodAttendanceScreen = () => {
  ...
  const handleSavePeriodAttendance = async () => {
    try {
      ...
      console.log("Sending period attendance payload:", payload);

      // Call the API
      const response = await bulkCreatePeriodAttendance(payload).unwrap();

      console.log("Period attendance submission response:", response);
            ...
            <View className="flex-row items-center mt-1">
              ...
              {remarks[item._id] && (
                <View className="bg-gray-200 rounded-full px-2 py-0.5 ml-1">
                  <Text className="text-xs text-gray-700">{remarks[item._id]}</Text>
                </View>
              )}
            </View>
            <TouchableOpacity
  ...
  const CustomDropdown = ({ 
    ...
    <View className="mb-4">
      ...
      >
        <Text className={`${value ? "text-gray-800" : "text-gray-500"} font-medium`}>
      ...
      {showDropdown && (
        <View className="border border-gray-300 rounded-lg mt-1 bg-white shadow-md max-h-60 z-10">
          {loading ? (
            <Text className="p-3 text-gray-500">Loading {label.toLowerCase()}...</Text>
          ) : options?.length > 0 ? (
            ...
            <Text className="p-3 text-gray-500">{noDataMessage || `No ${label.toLowerCase()} available`}</Text>
          )}
        </View>
      )}
    </View>
  ...
  const ListHeaderWithSearch = () => (
    <View className="px-4 pt-3 pb-2">
      <View className="bg-blue-50 rounded-xl p-4 mb-5 shadow-sm border border-blue-100">
        <Text className="text-blue-800 text-xl font-bold mb-1">Period Attendance</Text>
      ...
      <CustomDropdown 
        ...
        value={selectedClassId ? 
          `${classes?.data?.find(c => c._id === selectedClassId)?.name} - ${classes?.data?.find(c => c._id === selectedClassId)?.section}` 
          : null}
        ...
        getOptionLabel={(classItem) => `${classItem.name} - ${classItem.section}`}
        ...
        <>
          ...
          <View className="mb-5">
            ...
            <View className="flex-row justify-center items-center bg-white py-3 rounded-lg shadow-sm border border-gray-200">
              <TouchableOpacity
                ...
                className={`${periodIndex <= 1 ? 'bg-gray-100' : 'bg-blue-100'} w-12 h-12 rounded-full items-center justify-center mr-6`}
          ...
          <CustomDropdown 
            ...
            value={selectedSubjectId ?
              subjects?.find(s => s._id === selectedSubjectId)?.name || "Unknown Subject"
              : null}
            options={subjects || []}
            ...
            getOptionLabel={(subject) => `${subject.name}${subject.code ? ` (${subject.code})` : ''}`}
          ...
          <CustomDropdown 
            ...
            value={selectedTeacherId ?
              teachers?.data?.find(t => t._id === selectedTeacherId)?.userId?.name || "Unknown Teacher"
              : null}
            ...
            getOptionLabel={(teacher) => `${teacher.userId?.name}${teacher.employeeId ? ` (${teacher.employeeId})` : ''}`}
